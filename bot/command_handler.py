...
class Welcome(TelebotHandler):
    ...
    def __call__(self, update: Update, context: CallbackContext):
        send_msg = 'Привет {}! ' \
                   'Здесь ты найдешь лучшие предложения по поиску отелей ' \
                   'Для начала посмотри, что я умею /help'\
...
class History(TelebotHandler):
    ...
    def __call__(self, update: Update, context: CallbackContext) -> None:
        data_requests = DATABASE.read_requests(
            user_id=update.message.from_user.id
        )

        if len(data_requests) < COUNT_MAX_ACTION_HISTORY:
            data_start = DATABASE.read_user(
                user_id=update.message.from_user.id
            )
            update.message.reply_text(
                'Начало: {}'.format(data_start.date_request
                                    .strftime('%d.%m.%Y %H:%M'))
            )
...
class Help(TelebotHandler):
    ...
    def __call__(self, update: Update, context: CallbackContext):
        send_msg = ['/{}: {}'.format(text_command, obj_command.__doc__)
                    for text_command, obj_command
                    in TelebotHandler.COMMANDS.items()
                    ]
        send_msg = '\n'.join(send_msg)

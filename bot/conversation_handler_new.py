...
class StateBuilder(ABC):
    """ Базовй класс-строитель для динамического создания словаря состояния для обработчиков"""
    ...
    def command_handler(self):
        ...
        self.state.update({
            self.commands_cls.successor:
                [MessageHandler(filter_text, getattr(self, f'_{self.keys[1]}'))],
    ...
    def query_handler(self):
        ...
        """
        for step in ['check_in', 'check_out', 'search']:
            try:
                idx_step = self.keys.index(step)
            except ValueError as err:
                my_logger.warning(f'{self.command}| Conversation Handler. {err}')
    ...
    def msg_query_handler(self):
        """
        Добавление обработчиков для текстовых сообщений и кнопок
        """
        for step in ['people', 'hotel', 'photo']:
            try:
                idx_step = self.keys.index(step)
            except ValueError as err:
                my_logger.warning(f'{self.command}| Conversation Handler. {err}')
...
class SortPriceBuilder(StateBuilder):
    """ Класс-строитель для команд lowprice и highprice"""
    def __init__(self, command, commands_cls):
...
class BestDealBuilder(StateBuilder):
    """ Класс-строитель для команды bestdeal"""
    def __init__(self, command, commands_cls):
        super().__init__(command, commands_cls)
        self.keys = [
            'command_cls',
...
class ConversationDirector():
    ...
    def create_conversation_handler(self) -> ConversationHandler:
        ...
        self._builder.msg_handler()
        self._builder.query_handler()
        self._builder.msg_query_handler()
...
class SortPriceConversationHandler():
    ...
    def __init__(self, command, commands_cls):
        builder = SortPriceBuilder(command, commands_cls)
...
class BestdealConversationHandler():
    ...
    def __init__(self, command, commands_cls):
        builder = BestDealBuilder(command, commands_cls)
        self.director = ConversationDirector(builder)
